  method get_nfe.
    ...
    try.
        ...
        lo_request->set_query( 'invoiceKey=' && i_accessKey ).

        data(lo_response) = lo_http_client->execute( if_web_http_client=>get ).

        data(ls_http_status) = lo_response->get_status( ).

        e_json_string  = lo_response->get_text( ).

        e_json = cl_json_string_reader=>create( e_json_string ).
