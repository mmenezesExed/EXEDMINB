...
class lhc_tabs_operations definition.
  ...
  public section.
    ...
    class-data: is_processar type abap_boolean.
...
class lhc__nfemonitorh implementation.
  ...
  method get_instance_authorizations.
    ...
    result = value #( for line in lt_monitor_etapa ( ChaveNFe = line-ChaveNFe
                                                     %action = value #( etapa_700 = cond #( when line-Atividade eq 700 then if_abap_behv=>auth-allowed else if_abap_behv=>auth-unauthorized )
                                                                        etapa_800 = cond #( when line-Atividade eq 800 then if_abap_behv=>auth-allowed else if_abap_behv=>auth-unauthorized ) ) ) ).
  ...
  method etapa_700.
    ...
    loop at lt_header into data(ls_header).
      ...
      if ls_header-EntradaMercadoria is initial.
        ...
        if ls_failed-_nfemonitorh is not initial.
          ...
          if ls_failed-_nfemonitorh is not initial.
            ...
            where PurchaseOrder = @ls_item-Pedido
               and PurchaseOrderItem = @ls_item-ItemPedido
               and PurchasingHistoryCategory eq 'E'
            into @ls_header-EntradaMercadoria.
                  ...
                  exporting
                    i_historico = value #( IdNFe = ls_header-ChaveNFe
                                           ...
                                           Descricao = me->new_message( id = lhc_tabs_operations=>cc_classe_msg
                                                                        ...
                                                                        v1       = ls_header-EntradaMercadoria )->if_message~get_text( ) ) ).
          endif.
        endif.
        ...
        if ls_failed-_nfemonitorh is not initial.
          ...
          loop at ls_reported-_nfemonitorh into report_em.
            lhc_tabs_operations=>register_historico(
              exporting
                i_historico = value #( IdNFe = ls_header-ChaveNFe
                                       Etapa = ls_header-atividade
...
class lhc__nfemonitori implementation.
  ...
  method update.
    loop at entities into data(entitie).
      if entitie-PedidoItem is not initial.
        split entitie-PedidoItem at '/' into: entitie-Pedido entitie-ItemPedido.
        entitie-%control-Pedido = if_abap_behv=>mk-on.
        entitie-%control-ItemPedido = if_abap_behv=>mk-on.
      endif.
  ...
  method read.
    lhc_tabs_operations=>read_item_data( exporting it_keys  = corresponding #( keys )
