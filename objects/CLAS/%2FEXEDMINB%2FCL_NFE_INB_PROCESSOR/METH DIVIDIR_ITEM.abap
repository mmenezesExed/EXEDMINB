   method dividir_item.
     ...
     if it_keys[ 1 ]-%param-qCom = 0.
       failed-_nfemonitori = value #( ( %fail = value #( cause = if_abap_behv=>cause-unauthorized )
                                        %key = value #( chavenfe = ls_nfe_item_data-ChaveNFe )
                                        %action-dividir = if_abap_behv=>mk-on ) ).
       reported-_nfemonitori = value #( ( %key = value #( chavenfe = ls_nfe_item_data-ChaveNFe )
